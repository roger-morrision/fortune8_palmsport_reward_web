...
function PaypalDetails() {
  ...
  const { mutateAsync, isPending } = useMutationApi(
    async (result) => {
      ...
      if (result.params.serviceType === "PAYPAL") {
        ...
        payloadPayoutInstrument.attribute_payload = {
          paypal_email: result.params.paypalEmail,
          bank_account_number: result.params.achAccountNumber,
          bank_routing_number: result.params.achRoutingNumber,
  ...
  const handleSubmit = () => {
    ...
    if (!state.paypalEmail) {
      newError["paypalEmail"] = "Required";
    } else if (!validator.isEmail(state?.paypalEmail?.trim())) {
      newError["paypalEmail"] = "Invalid email format";
    }
        ...
        <View style={{ gap: 21, marginTop: 10 }}>
          <View>
            <TextInput
              ...
              style={[styles.input_style, error?.paypalEmail && styles.input_error_style]}
